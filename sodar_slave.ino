...
{
  unsigned long ms = millis(); /* millis() returns number of milliseconds since boot, 32 bit number */
...
void handleSpeed(int *cmdVector)
{
  cmdVector[0]=0;
  cmdVector[1]=sonar.ping();
  sendVector(2,cmdVector);
}

void handleDistance(int *cmdVector)
{
  cmdVector[0]=0;
  cmdVector[1]=sonar.ping_cm();
  sendVector(2,cmdVector);
}
...
void loop() {
  ...
  if (Serial.available()) { /* If MATLAB has sent a command, go get it */
    ...
    case 3: /* uptime */
     ...
     break;
    case 4: /* return ping time */
      handleSpeed(cmdVector);
      break;
     case 5: /*return distance*/
      handleDistance(cmdVector);
      break;
